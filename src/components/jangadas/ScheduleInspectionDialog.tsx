...
export default function ScheduleInspectionDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const agendaPayload = {
        title: `Inspeção: ${jangada.shipNameManual || jangada.owner || "Jangada"} - ${jangada.serial}`,
        raftSerial: jangada.serial,
        date: parsedDate.toISOString(),
        responsavel: tecnico || "Operador",
        status: "scheduled",
        type: "Inspeção",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col text-left">
        ...
        <div className="flex justify-end gap-2 border-t border-slate-100 bg-slate-50 px-6 py-4">
          ...
          >
            {loading ? "A agendar..." : "Gravar Agendamento"}
          </button>
        </div>
      </div>
    </div>
